...
\begin{comment}
Nel caso venga scelta l'utilizzo di un approccio senza Overload Management, l'utente sar\'a vincolato nell'inserimento di altri due parametri quali: \textit{\STOP_iniziale}, \textit{\STOP_finale} ed il \textit{Numero di Run}.
Questi parametri sono utilizzati al fine di calcolare il passo\footnote{Per ``passo'' si intende l'incremento unitario dell'istante conclusivo di simulazione durante un insieme di run, nel calcolo del comportamento \textit{steady-state}} di ogni esecuzione mediante la seguente formula:
...
Nel caso in cui l'utente scelga un approccio con Overload Management l'utente dovr\'a impostare manualmente il \textit{numero di batch} e la \textit{grandezza del batch} stesso.
